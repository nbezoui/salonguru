...
import { Observable, combineLatest, Subscription, map, take } from 'rxjs';
...
export class CartComponent implements OnInit, OnDestroy {
  ...
  checkout(): void {
    this.checkoutSubscription.add(
      combineLatest([
        this.cartItems$.pipe(map(cartItems => this.getCheckoutArray(cartItems)), take(1)),
        this.allProducts$.pipe(take(1))
      ]).subscribe(([checkout, allProducts]) => {
        this.store.dispatch(initiateCheckout({ items: checkout, products: allProducts }));
